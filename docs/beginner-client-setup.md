...
如果是在本机源码目录里试用，不想记命令，macOS 可以直接双击项目根目录的：
...
```bash
./scripts/start-desktop-macos.sh
```

脚本会自动准备 Python/Node 依赖，必要时重新构建 WebUI，然后启动桌面端。

## 3. 配置 AI 模型

首次打开客户端时，可以直接进入“首次配置”向导。按页面顺序填入：
...
点击“测试 LLM”确认连通后，点击“保存并开始分析”即可提交第一份报告任务。需要更细的渠道、新闻源或通知配置时，再进入“系统设置”。

如果没有看到向导，也可以从首页的“基础配置未完成”提示进入，或手动打开左侧设置里的高级配置。
